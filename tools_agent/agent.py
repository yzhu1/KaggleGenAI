...
currency_agent = LlmAgent(
    ...
    1. Print to stdout the line "=========In currentcy_agent=========" to indicate the agent is processing the request.
    2. Use `get_fee_for_payment_method()` to find transaction fees
    3. Use `get_exchange_rate()` to get currency conversion rates
    4. Check the "status" field in each tool's response for errors
    5. Calculate the final amount after fees based on the output from `get_fee_for_payment_method` and `get_exchange_rate` methods and provide a clear breakdown.
    6. First, state the final converted amount.
        Then, explain how you got that result by showing the intermediate amounts. Your explanation must include: the fee percentage and its
        value in the original currency, the amount remaining after the fee, and the exchange rate used for the final conversion.

    If any tool returns status "error", explain the issue to the user clearly.

    IMPORTANT: Call each tool exactly once. Do not retry or make redundant calls.
    """,
...
enhanced_currency_agent = LlmAgent(
  ...
  For any currency conversion request:
   ...
   4. Calculate Final Amount (CRITICAL): You are strictly prohibited from performing any arithmetic calculations yourself. You must find the final converted amount using the Python code generated by the calculation_agent tool. This code will use the fee information from step 1 and the exchange rate from step 2.
   5. Provide Detailed Breakdown: In your summary, you must:
       1. Use the output of the Python code in stdout to show the final result.
       2. State the final converted amount
       3. Explain how the result was calculated, including:
